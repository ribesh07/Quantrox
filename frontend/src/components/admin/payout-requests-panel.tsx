...
import { PayoutStatus, PayoutType } from "@/lib/prisma-types";
...
type AdminPayoutRequestsPanelProps = {
...
export function AdminPayoutRequestsPanel({
  ...
  description,
  revalidatePath,
}: AdminPayoutRequestsPanelProps) {
  const queryClient = useQueryClient();
  const statsKey = `admin-payout-stats-${payoutType}`;
  const listKey = `admin-payout-requests-${payoutType}`;
  ...
  const isMerchant = payoutType === PayoutType.MERCHANT;

  const { data: counts } = useQuery({
    queryKey: [statsKey],
    queryFn: async () => {
      const result = await getPayoutStatusCountsAction(payoutType);
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await getAllPayoutRequestsAction(statusFilter, payoutType);
  ...
  const approveMutation = useMutation({
    mutationFn: async (id: string) => {
      const result = await approvePayoutRequestAction(id, [revalidatePath]);
      if (!result.success) throw new Error(result.error);
  ...
  const rejectMutation = useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      const result = await rejectPayoutRequestAction(id, reason, [revalidatePath]);
      if (!result.success) throw new Error(result.error);
      return result.payout;
    },
    onSuccess: () => {
  ...
  const markPaidMutation = useMutation({
    mutationFn: async ({ id, hash, file }: { id: string; hash: string; file: File }) => {
      ...
      const result = await markPayoutPaidAction(id, formData, [revalidatePath]);
      if (!result.success) throw new Error(result.error);
      return result.payout;
    },
    onSuccess: () => {
  ...
  const filteredPayouts = Array.isArray(data)
    ? data.filter(
        (p: any) =>
          ...
          p.walletNetwork?.toLowerCase().includes(search.toLowerCase()) ||
  ...
  const methodLabel = isMerchant ? "Network" : "Method";
  const accountLabel = isMerchant ? "Wallet Address" : "UID";
          ...
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="relative w-full sm:w-80">
                ...
                <Input
                  placeholder={isMerchant ? "Search by user, wallet, network..." : "Search by user, UID, method..."}
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30 border-b">
                    ...
                    <TableHead className="font-bold">{methodLabel}</TableHead>
                    <TableHead className="font-bold">Amount</TableHead>
                    <TableHead className="font-bold">{accountLabel}</TableHead>
          ...
          {detailPayout && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  <p className="text-muted-foreground">{isMerchant ? "Network" : "Payment Method"}</p>
              ...
              {detailPayout.qrCodeImage && (
                <div>
                  <p className="text-sm text-muted-foreground mb-2">{isMerchant ? "Receiving QR (optional)" : "User Receiving QR"}</p>
