...
async function main() {
  ...
  const paymentMethods = [
    { name: 'USDT (TRC20)', category: PaymentMethodCategory.DEPOSIT, feePercentage: 0, rate: 1 },
    { name: 'USDT (BEP20)', category: PaymentMethodCategory.DEPOSIT, feePercentage: 0, rate: 1 },
    { name: 'USDT (ERC20)', category: PaymentMethodCategory.DEPOSIT, feePercentage: 0, rate: 1 },
    ...
    { name: 'Stripe', category: PaymentMethodCategory.DEPOSIT, feePercentage: 5, rate: 1 },
    { name: 'PayPal', category: PaymentMethodCategory.BOTH, feePercentage: 5, rate: 0.92 },
    { name: 'Apple Pay', category: PaymentMethodCategory.DEPOSIT, feePercentage: 5, rate: 1 },
  ];
    ...
    if (!existing) {
      await prisma.paymentAccount.create({
        data: {
          ...
          walletAddress: method.name.includes('USDT') ? 'TXXXXXXXXXXXXXXXXXXXXXXXXXXXX' : null,
  ...
  for (const user of users) {
    ...
    await prisma.transaction.create({
      data: {
        ...
        userId: user.id,
        amount: order.total,
        type: TransactionType.GAME_PURCHASE, // ✅ FIXED
        status: 'SUCCESS',
      },
    });
  ...
  for (const profile of merchantProfiles) {
    ...
    seededMerchants.push({ user: merchantUser });
  }
